...
test("coalesces concurrent startup and reuses one ready scope", async () => {
  ...
  assert.deepEqual(registry.snapshot().metrics, { starts: 1, shares: 2, failures: 0, mismatches: 0, shutdowns: 0, shutdown_failures: 0, scope_count: 1 });
...
test("retains a failed scope tombstone without retry", async () => {
  ...
  assert.deepEqual(registry.snapshot().metrics, { starts: 1, shares: 1, failures: 1, mismatches: 0, shutdowns: 0, shutdown_failures: 0, scope_count: 1 });
});
