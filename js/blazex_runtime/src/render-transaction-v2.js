import { validIntent, intentCount } from "./render-intent-data.js";
...
function preflight(tx, context) {
  ...
  let listenerCount = context.listeners.length;
  ...
  tx.operations.forEach((op, index) => {
    ...
    if (op.type === "create") {
      ...
      if (op.type === "insert" || op.type === "move") {
        ...
        require(validIntent(op.name, op.old) && validIntent(op.name, op.new));
        if (op.name === "listeners") listenerCount += intentCount(op.new) - intentCount(op.old);
        require(listenerCount >= 0 && listenerCount <= 256, "limit");
...
export async function validate(record, context) {
  ...
  require(value !== null && value.protocol === "blazex.dom-transaction/2" && value.schema === "2.0.0", "incompatible");
