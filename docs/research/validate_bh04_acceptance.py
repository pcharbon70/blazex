...
def validate(root=ROOT, final=True):
    ...
    def bindings(values):
        for file, expected in values.items():
            check(hashlib.sha256((root / file).read_bytes()).hexdigest() == expected, "stale binding: " + file)
    try:
        ...
        check(changed == [], "runtime/package/profile change outside measurement scope")
        if final:
            ...
            required |= {p.name for p in (root / "docs/research").glob("validate_*.py") if p.name != Path(__file__).name}
            required |= {p.name for p in (root / "docs/research").glob("generate_*.py")}
