...
- [Cross-platform native host and renderer architecture for BlazeX](cross-platform-native-host-and-renderer-architecture.md) — compares shell, layout, drawing, text, accessibility, direct platform controls, BEAM-boundary, and packaging strategies; it advances SDL3/Skia as the leading split-process custom-scene spike hypothesis plus separate Win32/AppKit/GTK actual-control proofs, excluding Qt and wxWidgets from active use.
