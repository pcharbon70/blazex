...
- [`validate_bh05_cleanup_scaling.py`](validate_bh05_cleanup_scaling.py) — Fail closed on Phase 13 scale-point/sample omissions and Phase 14 compact-outcome amplification, terminal-state relabelling, evidence hash drift and misleading execution/acceptance summaries.
...
- [`validate_bh05_scheduling.py`](validate_bh05_scheduling.py) — Phase 7 immutable boundary, source inventory and frozen scheduling evidence.
...
- [Recovery validator](validate_bh05_recovery.py) — Phase 10 immutable boundary, fixture inventory and frozen evidence.
...
- [BH-06 Phase 8 payload validator](validate_bh06_payload_budgets.py) — Recomputes public/private ownership, decoded/Brotli totals, budget decisions, evidence bindings, promotion rejection, and browser negotiation.
