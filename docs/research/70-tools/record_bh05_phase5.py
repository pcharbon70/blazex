"""Record all Phase 5 gates against an unchanged source closure; publish only passes."""
import argparse
import json
import os
...
from generate_bh05_nested import BASE, AREA
from validate_bh05_nested import completion, gate_errors, sources, validate


def run(args):
    revisions = [(args.phase4, BASE), (args.phase3, "fc5048d4db7cc80fd21b492e0638182abceda1af"), (args.phase2, "5cd382c23c5589404efc8dd7121432dd24c4cd96"),
    ...
    record = {"schema_version": "1.0.0", "phase": 5, "predecessor_revision": BASE,
              "environment": {"python": platform.python_version(), "node": subprocess.check_output(["node", "--version"], text=True).strip(), "elixir_image": "a2386c21edd5", "elixir": "1.17.3", "otp": "26.0.2", "scope": "ERTS nested state/reconciliation and headless oracle; no AtomVM execution parity"},
    ...
    docker = ["docker", "run", "--rm", "--network", "none", "--user", f"{os.getuid()}:{os.getgid()}", "-v", str(REPO_ROOT) + ":/workspace:ro", "-w", "/workspace", "-e", "MIX_BUILD_PATH=/tmp/bh05-phase5", "a2386c21edd5"]
    ...
    execute("nested-tests", [sys.executable, "-m", "unittest", "discover", "-s", "docs/research/70-tools", "-p", "test_validate_bh05_nested.py"])
    execute("nested-validator", [sys.executable, "docs/research/70-tools/validate_bh05_nested.py"])
    execute("nested-generator", [sys.executable, "docs/research/70-tools/generate_bh05_nested.py", "--check"])
...
def publish(output):
    ...
    for name, data in [("nested-gates-v0.1.0.json", record), ("nested-completion-v0.1.0.json", None)]:
    ...
    print("Phase 5 nested state/reconciliation complete; Phase 6 eligible, unauthorized.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    for name in ["output", "phase4", "phase3", "phase2", "phase1", "predecessor", "historical"]:
        parser.add_argument("--" + name, type=Path, required=name == "output")
    parser.add_argument("--publish", action="store_true")
    args = parser.parse_args()
    if args.publish: publish(args.output)
    elif not all([args.phase4, args.phase3, args.phase2, args.phase1, args.predecessor, args.historical]): parser.error("all replay paths required")
