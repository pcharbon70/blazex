"""Run the candidate subset through SHA-pinned Popcorn compiler/analyzer sources."""
import argparse
import hashlib
import io
import json
...
def run(package):
    ...
    with tempfile.TemporaryDirectory(prefix="bh05-subset-") as name:
        root = Path(name)
        ...
        command = ["docker", "run", "--rm", "--network", "none", "--user", f"{os.getuid()}:{os.getgid()}", "-v", str(REPO_ROOT) + ":/workspace:ro", "-v", name + ":/pinned", "-w", "/workspace", "a2386c21edd5", "elixir", "integration/bh-05/authoring-subset.exs", "/pinned"]
