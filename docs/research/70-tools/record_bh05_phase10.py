"""Record all Phase 10 gates against an unchanged source closure; publish only passes."""
import argparse
import json
import os
...
from generate_bh05_recovery import BASE, AREA
from validate_bh05_recovery import completion, gate_errors, sources, validate


def run(args):
    revisions = [(args.phase9, BASE), (args.phase8, "8e26ed110e1de2ab3be8039f27e5efaf7dea4117"), (args.phase7, "08dec098fde98b506d554bf8c3ff64bc65de6add"), (args.phase6, "e373e96891207ea4b6c2fdfae9533181334b3cab"), (args.phase5, "f2fdc745118f0ad84e5b3ab53ea9e5221310f957"), (args.phase4, "ea09d05a2709a163ff0c8776c4039f1a712a95de"), (args.phase3, "fc5048d4db7cc80fd21b492e0638182abceda1af"), (args.phase2, "5cd382c23c5589404efc8dd7121432dd24c4cd96"),
    ...
    record = {"schema_version": "1.0.0", "phase": 10, "predecessor_revision": BASE,
              "environment": {"python": platform.python_version(), "node": subprocess.check_output(["node", "--version"], text=True).strip(), "elixir_image": "a2386c21edd5", "elixir": "1.17.3", "otp": "26.0.2", "scope": "ERTS root failure containment, bounded retries and deadline-bound cleanup with independent headless oracle; AtomVM API/patch analysis only, no execution parity"},
    ...
    docker = ["docker", "run", "--rm", "--network", "none", "--user", f"{os.getuid()}:{os.getgid()}", "-v", str(REPO_ROOT) + ":/workspace:ro", "-w", "/workspace", "-e", "MIX_BUILD_PATH=/tmp/bh05-phase10", "a2386c21edd5"]
    ...
    execute("recovery-subset", [sys.executable, "docs/research/70-tools/check_bh05_recovery_subset.py"])
    execute("phase9-replay", ["bash", "-c", "python3 docs/research/70-tools/validate_bh05_scopes.py --final && python3 docs/research/70-tools/generate_bh05_scopes.py --check && python3 -m unittest discover -s docs/research/70-tools -p test_validate_bh05_scopes.py"], args.phase9)
    ...
    execute("recovery-tests", [sys.executable, "-m", "unittest", "discover", "-s", "docs/research/70-tools", "-p", "test_validate_bh05_recovery.py"])
    execute("recovery-validator", [sys.executable, "docs/research/70-tools/validate_bh05_recovery.py"])
    execute("recovery-generator", [sys.executable, "docs/research/70-tools/generate_bh05_recovery.py", "--check"])
...
def publish(output):
    ...
    for name, data in [("recovery-gates-v0.1.0.json", record), ("recovery-completion-v0.1.0.json", None)]:
    ...
    print("Phase 10 root recovery/cleanup complete; Phase 11 eligible, unauthorized.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    for name in ["output", "phase9", "phase8", "phase7", "phase6", "phase5", "phase4", "phase3", "phase2", "phase1", "predecessor", "historical"]:
        parser.add_argument("--" + name, type=Path, required=name == "output")
    parser.add_argument("--publish", action="store_true")
    args = parser.parse_args()
    if args.publish: publish(args.output)
    elif not all([args.phase9, args.phase8, args.phase7, args.phase6, args.phase5, args.phase4, args.phase3, args.phase2, args.phase1, args.predecessor, args.historical]): parser.error("all replay paths required")
