"""Isolated mutation tests for the Phase 3 successor evidence boundary."""
import copy
import json
...
from generate_bh05_schema import BASE, TARGET, PLAN
from validate_bh05_schema import GATES, NORMALIZED, files, gate_errors, validate


class SchemaEvidenceTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.directory = tempfile.TemporaryDirectory(prefix="bh05-schema-test-")
    ...
    def test_missing_or_changed_fixture(self):
        self.mutate("integration/bh-05/schema-fixtures.exs", None)
        self.mutate("integration/bh-05/schema-fixtures.exs", lambda data: data + b"\n# drift\n")
    ...
    def test_unknown_surface_and_future_completion(self):
        ...
        self.mutate(PLAN + "phase-04-pure-composition-and-atomic-semantic-evaluation.md", lambda data: data.replace(b"[ ]", b"[x]", 1))

    def test_inventory_rejects_runtime_credit(self):
        self.mutate("integration/bh-05/schema-index-v0.1.0.json", lambda data: data.replace(b'"component_execution": false', b'"component_execution": true'))
    ...
    def test_gate_failure_missing_duplicate_and_source_tampering(self):
        record = {"source_hashes": {"fixture": "hash"}, "final_source_hashes": {"fixture": "hash"}, "predecessor_revision": BASE,
                  "results": [{"name": name, "exit_code": 0, "error": None, "command": [name], "stdout": "NORMALIZED_SCHEMA_FIXTURE_SHA256 " + NORMALIZED if name in ["compile-fixtures", "schema-subset"] else ""} for name in GATES]}
