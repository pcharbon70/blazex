"""Isolated negative tests of the Phase 5 successor boundary and gate evidence."""
import copy
...
from generate_bh05_nested import BASE, TARGET, PLAN
from validate_bh05_nested import GATES, DIGESTS, files, gate_errors, validate


class NestedEvidenceTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.directory = tempfile.TemporaryDirectory(prefix="bh05-nested-test-")
    ...
    def test_authority_and_inherited_evidence(self):
        for path in [TARGET, "docs/research/assets/bh-05-baseline/composition-completion-v0.1.0.json"]:
    ...
    def test_inherited_kernel_dependencies_and_tools_immutable(self):
        for path in ["packages/blazex_core/lib/blazex/core/authoring.ex", "packages/blazex_ui_tree/mix.exs", "docs/research/70-tools/validate_bh05_composition.py"]:
            self.mutate(path, lambda data: data + b"\n# drift\n")

    def test_missing_changed_fixture_and_private_import(self):
        path = "integration/bh-05/nested-fixtures.exs"
    ...
    def test_unknown_surface_and_future_completion(self):
        ...
        self.mutate(PLAN + "phase-06-process-root-local-view-lifecycle-and-supervision.md", lambda data: data.replace(b"[ ]", b"[x]", 1))

    def test_inventory_rejects_unsupported_credit(self):
        self.mutate("integration/bh-05/nested-index-v0.1.0.json", lambda data: data.replace(b'"runtime_parity": false', b'"runtime_parity": true'))
    ...
    def test_gate_failure_missing_duplicate_stale_and_digest_tampering(self):
        record = {"source_hashes": {"fixture": "hash"}, "final_source_hashes": {"fixture": "hash"}, "predecessor_revision": BASE,
                  "results": [{"name": name, "exit_code": 0, "error": None, "command": [name], "stdout": "\n".join(k + " " + v for k, v in DIGESTS.items()) if name == "packages" else ""} for name in GATES]}
