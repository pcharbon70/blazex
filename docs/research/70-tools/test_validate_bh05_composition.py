"""Isolated negative tests of the Phase 4 successor boundary and gate evidence."""
import copy
...
from generate_bh05_composition import BASE, TARGET, PLAN
from validate_bh05_composition import GATES, DIGESTS, files, gate_errors, validate


class CompositionEvidenceTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.directory = tempfile.TemporaryDirectory(prefix="bh05-composition-test-")
    ...
    def test_authority_and_inherited_evidence(self):
        for path in [TARGET, "docs/research/assets/bh-05-baseline/schema-completion-v0.1.0.json"]:
    ...
    def test_inherited_kernel_dependencies_and_tools_immutable(self):
        for path in ["packages/blazex_core/lib/blazex/core/authoring.ex", "packages/blazex_ui_tree/mix.exs", "docs/research/70-tools/validate_bh05_schema.py"]:
            self.mutate(path, lambda data: data + b"\n# drift\n")

    def test_missing_changed_fixture_and_private_import(self):
        path = "integration/bh-05/composition-fixtures.exs"
    ...
    def test_unknown_surface_and_future_completion(self):
        ...
        self.mutate(PLAN + "phase-05-nested-stateful-identity-and-update-reconciliation.md", lambda data: data.replace(b"[ ]", b"[x]", 1))

    def test_inventory_rejects_unsupported_credit(self):
        self.mutate("integration/bh-05/composition-index-v0.1.0.json", lambda data: data.replace(b'"runtime_parity": false', b'"runtime_parity": true'))
