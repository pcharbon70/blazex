"""Isolated negative tests of the Phase 9 successor boundary and gate evidence."""
import copy
...
from generate_bh05_scopes import BASE, TARGET, PLAN
from validate_bh05_scopes import GATES, DIGESTS, RAW_MARKERS, files, gate_errors, validate


class ScopeEvidenceTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.directory = tempfile.TemporaryDirectory(prefix="bh05-scope-test-")
    ...
    def test_authority_and_inherited_evidence(self):
        for path in [TARGET, "docs/research/assets/bh-05-baseline/action-completion-v0.1.0.json"]:
    ...
    def test_missing_changed_fixture_and_private_import(self):
        path = "integration/bh-05/scope-fixtures.exs"
    ...
    def test_unknown_surface_and_future_completion(self):
        ...
        self.mutate(PLAN + "phase-10-failure-containment-retry-replacement-and-disposal.md", lambda data: data.replace(b"[ ]", b"[x]", 1))

    def test_inventory_rejects_unsupported_credit(self):
        self.mutate("integration/bh-05/scope-index-v0.1.0.json", lambda data: data.replace(b'"runtime_parity": false', b'"runtime_parity": true'))

    def test_gate_failure_missing_duplicate_stale_and_digest_tampering(self):
        record = {"phase": 9, "source_hashes": {"fixture": "hash"}, "final_source_hashes": {"fixture": "hash"}, "predecessor_revision": BASE,
