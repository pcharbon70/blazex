"""Record all Phase 8 gates against an unchanged source closure; publish only passes."""
import argparse
import json
import os
...
from generate_bh05_actions import BASE, AREA
from validate_bh05_actions import completion, gate_errors, sources, validate


def run(args):
    revisions = [(args.phase7, BASE), (args.phase6, "e373e96891207ea4b6c2fdfae9533181334b3cab"), (args.phase5, "f2fdc745118f0ad84e5b3ab53ea9e5221310f957"), (args.phase4, "ea09d05a2709a163ff0c8776c4039f1a712a95de"), (args.phase3, "fc5048d4db7cc80fd21b492e0638182abceda1af"), (args.phase2, "5cd382c23c5589404efc8dd7121432dd24c4cd96"),
    ...
    record = {"schema_version": "1.0.0", "phase": 8, "predecessor_revision": BASE,
              "environment": {"python": platform.python_version(), "node": subprocess.check_output(["node", "--version"], text=True).strip(), "elixir_image": "a2386c21edd5", "elixir": "1.17.3", "otp": "26.0.2", "scope": "ERTS typed actions and bounded resources and independent headless oracle; AtomVM API/patch analysis only, no execution parity"},
    ...
    docker = ["docker", "run", "--rm", "--network", "none", "--user", f"{os.getuid()}:{os.getgid()}", "-v", str(REPO_ROOT) + ":/workspace:ro", "-w", "/workspace", "-e", "MIX_BUILD_PATH=/tmp/bh05-phase8", "a2386c21edd5"]
    ...
    execute("action-subset", [sys.executable, "docs/research/70-tools/check_bh05_action_subset.py"])
    ...
    execute("action-tests", [sys.executable, "-m", "unittest", "discover", "-s", "docs/research/70-tools", "-p", "test_validate_bh05_actions.py"])
    execute("action-validator", [sys.executable, "docs/research/70-tools/validate_bh05_actions.py"])
    execute("action-generator", [sys.executable, "docs/research/70-tools/generate_bh05_actions.py", "--check"])
...
def publish(output):
    ...
    for name, data in [("action-gates-v0.1.0.json", record), ("action-completion-v0.1.0.json", None)]:
    ...
    print("Phase 8 typed actions/resources/command intent complete; Phase 9 eligible, unauthorized.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    for name in ["output", "phase7", "phase6", "phase5", "phase4", "phase3", "phase2", "phase1", "predecessor", "historical"]:
        parser.add_argument("--" + name, type=Path, required=name == "output")
    parser.add_argument("--publish", action="store_true")
    args = parser.parse_args()
    if args.publish: publish(args.output)
    elif not all([args.phase7, args.phase6, args.phase5, args.phase4, args.phase3, args.phase2, args.phase1, args.predecessor, args.historical]): parser.error("all replay paths required")
