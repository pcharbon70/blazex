"""Isolated negative tests of the Phase 7 successor boundary and gate evidence."""
import copy
...
from generate_bh05_scheduling import BASE, TARGET, PLAN
from validate_bh05_scheduling import GATES, DIGESTS, RAW_MARKERS, files, gate_errors, validate


class SchedulingEvidenceTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.directory = tempfile.TemporaryDirectory(prefix="bh05-scheduling-test-")
    ...
    def test_authority_and_inherited_evidence(self):
        for path in [TARGET, "docs/research/assets/bh-05-baseline/root-completion-v0.1.0.json"]:
    ...
    def test_missing_changed_fixture_and_private_import(self):
        path = "integration/bh-05/scheduling-fixtures.exs"
    ...
    def test_unknown_surface_and_future_completion(self):
        ...
        self.mutate(PLAN + "phase-08-effects-resources-and-typed-command-intent.md", lambda data: data.replace(b"[ ]", b"[x]", 1))

    def test_inventory_rejects_unsupported_credit(self):
        self.mutate("integration/bh-05/scheduling-index-v0.1.0.json", lambda data: data.replace(b'"runtime_parity": false', b'"runtime_parity": true'))

    def test_gate_failure_missing_duplicate_stale_and_digest_tampering(self):
        record = {"phase": 7, "source_hashes": {"fixture": "hash"}, "final_source_hashes": {"fixture": "hash"}, "predecessor_revision": BASE,
                  "results": [{"name": name, "exit_code": 0, "error": None, "command": [name], "stdout": "\n".join(k + " " + v for k, v in {**DIGESTS, **RAW_MARKERS}.items()) if name == "packages" else ""} for name in GATES]}
