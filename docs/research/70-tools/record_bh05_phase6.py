"""Record all Phase 6 gates against an unchanged source closure; publish only passes."""
import argparse
import json
import os
...
from generate_bh05_roots import BASE, AREA
from validate_bh05_roots import completion, gate_errors, sources, validate


def run(args):
    revisions = [(args.phase5, BASE), (args.phase4, "ea09d05a2709a163ff0c8776c4039f1a712a95de"), (args.phase3, "fc5048d4db7cc80fd21b492e0638182abceda1af"), (args.phase2, "5cd382c23c5589404efc8dd7121432dd24c4cd96"),
    ...
    record = {"schema_version": "1.0.0", "phase": 6, "predecessor_revision": BASE,
              "environment": {"python": platform.python_version(), "node": subprocess.check_output(["node", "--version"], text=True).strip(), "elixir_image": "a2386c21edd5", "elixir": "1.17.3", "otp": "26.0.2", "scope": "ERTS supervised roots and independent headless oracle; AtomVM API/patch analysis only, no execution parity"},
    ...
    docker = ["docker", "run", "--rm", "--network", "none", "--user", f"{os.getuid()}:{os.getgid()}", "-v", str(REPO_ROOT) + ":/workspace:ro", "-w", "/workspace", "-e", "MIX_BUILD_PATH=/tmp/bh05-phase6", "a2386c21edd5"]
    ...
    execute("root-subset", [sys.executable, "docs/research/70-tools/check_bh05_root_subset.py"])
    ...
    execute("root-tests", [sys.executable, "-m", "unittest", "discover", "-s", "docs/research/70-tools", "-p", "test_validate_bh05_roots.py"])
    execute("root-validator", [sys.executable, "docs/research/70-tools/validate_bh05_roots.py"])
    execute("root-generator", [sys.executable, "docs/research/70-tools/generate_bh05_roots.py", "--check"])
...
def publish(output):
    ...
    for name, data in [("root-gates-v0.1.0.json", record), ("root-completion-v0.1.0.json", None)]:
    ...
    print("Phase 6 supervised root lifecycle complete; Phase 7 eligible, unauthorized.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    for name in ["output", "phase5", "phase4", "phase3", "phase2", "phase1", "predecessor", "historical"]:
        parser.add_argument("--" + name, type=Path, required=name == "output")
    parser.add_argument("--publish", action="store_true")
    args = parser.parse_args()
    if args.publish: publish(args.output)
    elif not all([args.phase5, args.phase4, args.phase3, args.phase2, args.phase1, args.predecessor, args.historical]): parser.error("all replay paths required")
