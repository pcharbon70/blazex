"""Record the source-frozen Phase 2 gate; publish only an unchanged passing run."""
import argparse
import json
import os
...
from generate_bh05_authoring import BASE, AREA
from validate_bh05_authoring import completion, gate_errors, sources, validate
...
def run(args):
    for root, revision in [(args.phase1, BASE), (args.predecessor, BH04), (args.historical, HISTORICAL)]:
    ...
    record = {"schema_version": "1.0.0", "phase": 2, "predecessor_revision": BASE,
    ...
    docker = ["docker", "run", "--rm", "--network", "none", "--user", f"{os.getuid()}:{os.getgid()}", "-v", str(REPO_ROOT) + ":/workspace:ro", "-w", "/workspace", "-e", "MIX_BUILD_PATH=/tmp/bh05-phase2", "a2386c21edd5"]
    ...
    execute("compile-fixtures", docker + ["sh", "-c", "mix format --check-formatted integration/bh-05/*.exs && elixir integration/bh-05/authoring-check.exs"])
    execute("atomvm-subset", [sys.executable, "docs/research/70-tools/check_bh05_subset.py", "--package", str(args.package)])
    ...
    execute("authoring-tests", [sys.executable, "-m", "unittest", "discover", "-s", "docs/research/70-tools", "-p", "test_validate_bh05_authoring.py"])
    execute("authoring-validator", [sys.executable, "docs/research/70-tools/validate_bh05_authoring.py"])
    execute("authoring-generator", [sys.executable, "docs/research/70-tools/generate_bh05_authoring.py", "--check"])
...
def publish(output):
    ...
    for name, data in [("authoring-gates-v0.1.0.json", record), ("authoring-completion-v0.1.0.json", None)]:
    ...
    print("Phase 2 candidate contract complete; Phase 3 eligible, unauthorized.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    for name in ["output", "phase1", "predecessor", "historical", "package"]:
        parser.add_argument("--" + name, type=Path, required=name == "output")
    parser.add_argument("--publish", action="store_true")
    args = parser.parse_args()
    if args.publish: publish(args.output)
    elif not all([args.phase1, args.predecessor, args.historical, args.package]): parser.error("all replay paths and pinned package required")
