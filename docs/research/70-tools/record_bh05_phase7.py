"""Record all Phase 7 gates against an unchanged source closure; publish only passes."""
import argparse
import json
import os
...
from generate_bh05_scheduling import BASE, AREA
from validate_bh05_scheduling import completion, gate_errors, sources, validate


def run(args):
    revisions = [(args.phase6, BASE), (args.phase5, "f2fdc745118f0ad84e5b3ab53ea9e5221310f957"), (args.phase4, "ea09d05a2709a163ff0c8776c4039f1a712a95de"), (args.phase3, "fc5048d4db7cc80fd21b492e0638182abceda1af"), (args.phase2, "5cd382c23c5589404efc8dd7121432dd24c4cd96"),
    ...
    record = {"schema_version": "1.0.0", "phase": 7, "predecessor_revision": BASE,
              "environment": {"python": platform.python_version(), "node": subprocess.check_output(["node", "--version"], text=True).strip(), "elixir_image": "a2386c21edd5", "elixir": "1.17.3", "otp": "26.0.2", "scope": "ERTS bounded scheduling and independent headless oracle; AtomVM API/patch analysis only, no execution parity"},
    ...
    docker = ["docker", "run", "--rm", "--network", "none", "--user", f"{os.getuid()}:{os.getgid()}", "-v", str(REPO_ROOT) + ":/workspace:ro", "-w", "/workspace", "-e", "MIX_BUILD_PATH=/tmp/bh05-phase7", "a2386c21edd5"]
    ...
    execute("scheduling-subset", [sys.executable, "docs/research/70-tools/check_bh05_scheduling_subset.py"])
    ...
    execute("scheduling-tests", [sys.executable, "-m", "unittest", "discover", "-s", "docs/research/70-tools", "-p", "test_validate_bh05_scheduling.py"])
    execute("scheduling-validator", [sys.executable, "docs/research/70-tools/validate_bh05_scheduling.py"])
    execute("scheduling-generator", [sys.executable, "docs/research/70-tools/generate_bh05_scheduling.py", "--check"])
...
def publish(output):
    ...
    for name, data in [("scheduling-gates-v0.1.0.json", record), ("scheduling-completion-v0.1.0.json", None)]:
    ...
    print("Phase 7 bounded local scheduling complete; Phase 8 eligible, unauthorized.")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=__doc__)
    for name in ["output", "phase6", "phase5", "phase4", "phase3", "phase2", "phase1", "predecessor", "historical"]:
        parser.add_argument("--" + name, type=Path, required=name == "output")
    parser.add_argument("--publish", action="store_true")
    args = parser.parse_args()
    if args.publish: publish(args.output)
    elif not all([args.phase6, args.phase5, args.phase4, args.phase3, args.phase2, args.phase1, args.predecessor, args.historical]): parser.error("all replay paths required")
