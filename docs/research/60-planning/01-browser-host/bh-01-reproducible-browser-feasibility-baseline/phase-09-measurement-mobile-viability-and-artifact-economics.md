...
- [ ] 9 Phase - Measurement, Mobile Viability, and Artifact Economics.
  ...
  Measure build, payload, startup, interaction, memory, reliability, and
  constrained-mobile behavior from the canonical artifacts and stable browser
  scenarios, then determine whether the candidate has a bounded product-viable
  path.
  ...
  - [ ] 9.2 Section - Execute controlled desktop measurements.
    ...
    Produce reproducible raw distributions under fingerprinted desktop
    environments before comparing constrained devices.
    ...
    - [ ] 9.2.1 Task - Run startup and payload experiments.
      ...
      - [ ] 9.2.1.3 Subtask - Repeat a subset on a second controlled desktop environment to distinguish machine sensitivity from browser/runtime behavior.
  ...
  - [ ] 9.3 Section - Execute constrained-mobile measurements.

    Treat real mobile hardware and network constraints as a stop/go input, not
    as an optional desktop-emulation exercise.

    - [ ] 9.3.1 Task - Fingerprint and stabilize mobile runs.
      ...
      - [ ] 9.3.1.1 Subtask - Record physical device/model/architecture, OS/browser, memory/storage, battery/power, thermal state, display/input, background activity, network shaping, automation, and metric availability.
      - [ ] 9.3.1.2 Subtask - Define cooldown, charging, foreground, cache reset, network, retry, failure retention, and drift checks without substituting desktop emulation for required evidence.

    - [ ] 9.3.2 Task - Run mobile startup, interaction, and resource experiments.

      The subset must directly cover the mobile proof obligation and dominant
      product risks.

      - [ ] 9.3.2.1 Subtask - Measure cold/warm startup, Wasm instantiate, runtime/root readiness, payload/network, fallback, local interaction/paint, and authenticated command.
      - [ ] 9.3.2.2 Subtask - Measure memory/pages, repeated interaction, cleanup, background/foreground or navigation where governed, errors/crashes, long tasks, and thermal/power caveats.
      - [ ] 9.3.2.3 Subtask - Repeat enough samples across required Android and iOS/iPadOS candidate scenarios to classify unavailable, insufficient, failed, conditional, or observed results honestly.
  ...
  - [ ] 9.5 Section - Compare observations with budgets and stop conditions.
    ...
    - [ ] 9.5.1 Task - Produce statistical budget evaluations.
      ...
      - [ ] 9.5.1.1 Subtask - Generate per-environment and aggregate observed/pass/fail/insufficient/blocked records for build, payload, startup, interaction, server, resource, cleanup, and reliability budgets.
      - [ ] 9.5.1.2 Subtask - Review sample adequacy, harness overhead, environment variance, outliers, failures, tail percentiles, automation gaps, and mobile confidence with quality owners.
    ...
    - [ ] 9.5.2 Task - Decide mobile and overall performance viability.

      The candidate proceeds only with acceptable results or bounded reviewed
      mitigations that preserve architecture and reproducibility.

      - [ ] 9.5.2.1 Subtask - Evaluate `BX-BH01-PROOF-MOBILE-MEASUREMENT`, mobile-performance risk, payload/artifact risk, and relevant non-waivable blockers.
      - [ ] 9.5.2.2 Subtask - Record proceed/conditional/repeat/stop recommendation, required mitigations, owner, expiry, repeated phases, and prohibited support claims.
  ...
  - [ ] 9.6 Section - Phase 9 Integration Tests and Completion Evidence.
    ...
    - [ ] 9.6.1 Task - Run deterministic measurement and report gates.
      ...
      - [ ] 9.6.1.3 Subtask - Rerun representative desktop/mobile measurements and compare within governed variance/reproducibility rules.
    ...
    - [ ] 9.6.2 Task - Review viability and publish phase evidence.
      ...
      - [ ] 9.6.2.1 Subtask - Review payload, build, desktop/mobile startup, interaction, command, memory, reliability, cleanup, mitigation, and budget outcomes with product/quality/runtime/build owners.
      - [ ] 9.6.2.2 Subtask - Publish Phase 9 evidence with exact tools/environments, raw/report/artifact hashes, statistics, failures, budget outcomes, mobile proof, risks, stop/go decision, and limitations.
...
truthful stop decision. Do not discard failed samples, substitute emulation for
required device evidence, or lower a threshold merely to proceed.
