...
- [Erlang/OTP ERTS build, runtime, and source architecture](erlang-otp-project-2026-erts-build-runtime-and-source.md) — records the immutable OTP 29.0.6 / ERTS 17.0.6 source pin, documented cross-build path, mandatory-thread requirement, interpreter/JIT split, system layers, memory, ports, and native-extension evidence for a browser port.
- [Erlang/OTP boot, security, and browser compatibility](erlang-otp-project-2026-otp-boot-security-and-compatibility.md) — traces ERTS startup, embedded OTP releases, `kernel`/`stdlib`, supervision, code-loader authority, hostile-data risk, and browser security implications.
- [Emscripten browser porting and runtime environment](emscripten-project-2026-browser-porting-runtime.md) — documents the C/LLVM toolchain, cooperative event loop, pthread Workers, shared memory, cross-origin isolation, virtual filesystem, and platform limitations relevant to ERTS.
- [The BEAM Book](stenman-2025-beam-book.md) — supplies the primary internals guide to BEAM loading and execution, processes, reductions, schedulers, allocators, garbage collection, I/O, ports, and native extensions.
- [Efficient Memory Management for Concurrent Programs that Use Message Passing](sagonas-wilhelmsson-2006-erlang-memory-management.md) — provides peer-reviewed evidence on process-local heaps, copied messages, shared areas, synchronization, and low-pause collection.
- [Browsix: Bridging the Gap Between Unix and the Browser](powers-vilk-berger-2017-browsix.md) — demonstrates both the feasibility and scope of recreating process, signal, socket, pipe, and filesystem abstractions behind a browser host boundary.
- [Not So Fast: Analyzing the Performance of WebAssembly vs. Native Code](jangda-et-al-2019-webassembly-performance.md) — shows why representative whole-runtime, per-browser measurement is necessary and historical small-kernel performance claims are insufficient.
- [Everything Old is New Again: Binary Security of WebAssembly](lehmann-kinder-pradel-2020-webassembly-binary-security.md) — establishes that unsafe-language corruption can remain exploitable inside Wasm linear memory and can gain host effects through powerful imports.
- [Swivel: Hardening WebAssembly against Spectre](narayan-et-al-2021-swivel.md) — uses Lucet's native-x86 embedding to show that sequential Wasm guarantees alone do not establish speculative confidentiality; it is not a browser/ERTS mitigation result.
- [Scaling Reliably: Improving the Scalability of the Erlang Distributed Actor Platform](trinder-et-al-2017-scaling-reliably-erlang.md) — supplies peer-reviewed ERTS evidence on scheduler balancing, time and timers, ETS contention, shared state, and multidimensional scalability measurement.
- [WebAssembly browser security, isolation, and threading standards](webassembly-standards-2026-browser-security-and-threads.md) — records import-scoped authority, Worker/shared-memory semantics, COOP/COEP, CSP integration, and the limits of browser Wasm guarantees.
- [WASI and WebAssembly threading proposal status](webassembly-project-2026-wasi-and-threading-status.md) — distinguishes Core shared-memory atomics, legacy `wasi-threads`, evolving Shared-Everything Threads, and the browser embedder work none of them removes.
