...
const child = spawn("docker", ["run", "--rm", "-i", "--network", "none", "-e", "MIX_BUILD_PATH=/tmp/bh04-phase5-browser", "-v", `${root}:/workspace`, "-w", "/workspace/integration/conformance", "a2386c21edd5", "mix", "run", "../bh-04/support/interaction_runtime.exs"], { stdio: ["pipe", "pipe", "inherit"] });
let serial = 0, ready;
...
lines.on("line", line => {
  if (line === "BH05:ready") { ready(); return; }
  if (!line.startsWith("BH05:")) return;
...
try {
  ...
  for (const [name, launcher, executable] of [["chrome", chromium, "/usr/bin/google-chrome"], ["firefox", firefox, process.env.BH04_FIREFOX ?? "/home/ducky/.cache/ms-playwright/firefox-1538/firefox/firefox"]]) {
    ...
    try {
      ...
      await page.exposeFunction("bh04Click", async point => { await page.mouse.click(point.x, point.y); });
      await page.goto(`http://127.0.0.1:${server.address().port}/`);
      const result = await page.evaluate(async () => {
        const { runInteractionScenarios } = await import("/integration/bh-04/interaction-scenarios.js");
        return runInteractionScenarios(document, globalThis.bh04Runtime, globalThis.bh04Click);
      });
  ...
  if (process.argv[2]) fs.writeFileSync(process.argv[2], JSON.stringify({ schema_version: "1.0.0", phase: 5, platform: "linux", date: new Date().toISOString(), execution: "Elixir 1.17.3 / OTP 26 in offline Docker; test-only DevTools/stdio carrier", results }, null, 2) + "\n");
