...
Code.require_file("continuity_component.exs", __DIR__)

defmodule BlazeX.BH04.ContinuityRuntime do
  ...
  def run(states \\ %{}) do
    case IO.read(:stdio, :line) do
      ...
      line ->
        ...
        IO.puts("BH06:" <> Jason.encode!(%{"id" => id, "result" => output}))
        run(next)
    end
  end
  defp execute(states, %{"control" => "init", "root_id" => root, "lifecycle_generation" => generation}) do
    ...
    {:ok, state, tx} = ContinuitySession.mount(root, generation, BlazeX.BH04.ContinuityComponent, owner, %{})
    {Map.put(states, root, state), %{"transaction" => tx, "events" => Enum.map(Event.names(), &Atom.to_string/1)}}
  end
  ...
  defp execute(states, %{"control" => "snapshot", "root_id" => root}) do
    state = Map.fetch!(states, root)
    {states, %{"count" => if(state.evaluation, do: state.evaluation.state, else: nil), "sequence" => state.last_sequence, "revision" => if(state.context, do: state.context["revision"], else: nil), "disposed" => state.disposed, "pending" => state.pending_sequence}}
  end
  defp execute(states, %{"protocol" => "blazex.host-bridge/3", "root_id" => root} = request) do
...
IO.puts("BH06:ready")
BlazeX.BH04.ContinuityRuntime.run()
