...
path = Path.expand("../protocol-fixtures-v0.1.0.txt", __DIR__)
results = for line <- File.stream!(path, [], :line) do
  [name, expected, context64, record64] = line |> String.trim() |> String.split("|")
  {:ok, context} = context64 |> Base.decode64!() |> Codec.decode()
  result = with {:ok, record} <- record64 |> Base.decode64!() |> Codec.decode(),
                {:ok, value} <- Protocol.new(record, context), do: {:ok, Protocol.to_wire(value)}
