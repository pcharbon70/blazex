...
const child = spawn("docker", ["run", "--rm", "-i", "--network", "none", "-e", "MIX_BUILD_PATH=/tmp/bh04-phase7-browser", "-v", `${root}:/workspace`, "-w", "/workspace/integration/conformance", "a2386c21edd5", "mix", "run", "../bh-04/support/effect_runtime.exs"], { stdio: ["pipe", "pipe", "inherit"] });
let serial = 0, ready;
...
lines.on("line", line => {
  if (line === "BH07:ready") { ready(); return; }
  if (!line.startsWith("BH07:")) return;
...
try {
  ...
  for (const [name, launcher, executable] of [["chrome", chromium, "/usr/bin/google-chrome"], ["firefox", firefox, process.env.BH04_FIREFOX ?? "/home/ducky/.cache/ms-playwright/firefox-1538/firefox/firefox"]]) {
    ...
    try {
      ...
      const result = await page.evaluate(async () => {
        const { runEffectScenarios } = await import("/integration/bh-04/effect-scenarios.js");
        return runEffectScenarios(document, globalThis.bh04Runtime, globalThis.bh04Click);
      });
  ...
  if (process.argv[2]) fs.writeFileSync(process.argv[2], JSON.stringify({ schema_version: "1.0.0", phase: 7, platform: "linux", date: new Date().toISOString(), execution: "Elixir 1.17.3 / OTP 26 in offline Docker; test-only DevTools/stdio carrier", results }, null, 2) + "\n");
