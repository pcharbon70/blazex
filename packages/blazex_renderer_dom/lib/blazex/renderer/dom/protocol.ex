defmodule BlazeX.Renderer.DOM.Protocol do
  @moduledoc """
  ...
  """
  alias BlazeX.Renderer.DOM.Protocol.{Codec, Schema}
  import Codec, only: [require!: 1, require!: 2]
  @enforce_keys [:record]
  defstruct [:record]
  @protocol "blazex.dom-transaction/1"
  @schema "1.0.0"
  ...
  defp preflight(tx, context) do
    ...
    if tx["kind"] == "dispose" do
      ...
      {nodes, detached, _listeners} =
        tx["operations"]
        |> Enum.with_index()
        |> Enum.reduce({nodes, MapSet.new(), MapSet.new(context["listeners"])}, fn {op, index},
                                                                                   state ->
          require!(
  ...
  defp apply_operation(op, _, _, {nodes, detached, listeners}) do
    ...
    case op["type"] do
      ...
      "listener" ->
        require!(
          op["old"] != op["new"] and MapSet.member?(listeners, op["listener"]) == op["old"],
          "stale"
        )

        updated =
          if op["new"],
            do: MapSet.put(listeners, op["listener"]),
            else: MapSet.delete(listeners, op["listener"])

        require!(MapSet.size(updated) <= 256, "limit")
        {nodes, detached, updated}
      ...
      "focus" ->
        if op["old"] != nil, do: target!(nodes, op["old"])
        {nodes, detached, listeners}

      "selection" ->
        require!(Enum.all?([op["old"], op["new"]], &(is_nil(&1) or &1["start"] <= &1["end"])))
        {nodes, detached, listeners}
