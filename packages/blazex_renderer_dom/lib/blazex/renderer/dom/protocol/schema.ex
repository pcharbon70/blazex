defmodule BlazeX.Renderer.DOM.Protocol.Schema do
  @moduledoc false
  # Generated from integration/bh-04/render-transaction.schema.json.
  @schemas %{
    "transaction" => %{
      "type" => "object",
      "properties" => %{
        "record" => %{"enum" => ["transaction"]},
        "protocol" => %{"enum" => ["blazex.dom-transaction/1"]},
        "schema" => %{"enum" => ["1.0.0"]},
                  ...
                  "old" => %{
                    "oneOf" => [
                      ...
                      %{
                        "type" => "object",
                        "properties" => %{
                          "start" => %{"type" => "integer", "minimum" => 0, "maximum" => 2048},
                          "end" => %{"type" => "integer", "minimum" => 0, "maximum" => 2048},
  ...
  def valid?(value, %{"oneOf" => options}), do: Enum.count(options, &valid?(value, &1)) == 1
  def valid?(value, %{"enum" => values}), do: value in values
  def valid?(value, %{"type" => "null"}), do: is_nil(value)
  def valid?(value, %{"type" => "boolean"}), do: is_boolean(value)

  def valid?(value, %{"type" => "integer", "minimum" => min, "maximum" => max}),
    do: is_integer(value) and value >= min and value <= max

  def valid?(value, %{"type" => "string"} = schema),
    do:
  ...
  def valid?(value, %{"type" => "array"} = schema),
    do:
      is_list(value) and length(value) >= Map.get(schema, "minItems", 0) and
        length(value) <= schema["maxItems"] and Enum.all?(value, &valid?(&1, schema["items"]))

  def valid?(value, %{"type" => "object"} = schema),
    do:
      is_map(value) and not is_struct(value) and map_size(value) == length(schema["required"]) and
        Enum.all?(
          schema["required"],
          &(Map.has_key?(value, &1) and valid?(value[&1], schema["properties"][&1]))
        )
