defmodule BlazeX.Renderer.DOM.InteractionSession do
  ...
  alias BlazeX.UITree.{ComponentEvaluator, Document, IntentSet}
  ...
  def mount(root_id, lifecycle_generation, component, identity, props) do
    with true <- is_binary(root_id) and Regex.match?(~r/\A[a-z][a-z0-9-]{0,63}\z/, root_id),
         ...
         {:ok, evaluation} <- ComponentEvaluator.mount(component, identity, props),
         {:ok, renderer} <- ReconciledSession.mount(evaluation.output) do
      state = %__MODULE__{
        root_id: root_id,
      ...
      {:ok, state, ReconciledSession.transaction(renderer)}
    else
      _ -> {:error, "mount"}
    end
  end
  ...
  def acknowledge(
    ...
    tx = ReconciledSession.transaction(state.renderer)

    with {:ok, renderer} <- ReconciledSession.acknowledge(state.renderer, ack),
  ...
  def deliver(%__MODULE__{} = state, record) do
    with :ok <- validate(record),
         ...
         {:ok, candidate, []} <- ComponentEvaluator.dispatch(state.evaluation, event) do
      if candidate.output == state.evaluation.output do
        ...
        case ReconciledSession.update(state.renderer, candidate.output) do
          {:ok, renderer} ->
            {:ok,
             %{
               state
             ...
             %{
               ...
               "transaction" => ReconciledSession.transaction(renderer)
             }}
  ...
  def handle(%__MODULE__{} = state, request) do
    with true <- exact?(request, ~w(protocol root_id request_id operation payload)),
         true <-
           request["protocol"] == "blazex.host-bridge/2" and request["root_id"] == state.root_id,
